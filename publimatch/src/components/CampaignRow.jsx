...
import { Box, Typography, IconButton, Card, Avatar, Tooltip } from '@mui/material';
  ...
  return (
    <Card
     onClick={() => navigate(`/campaign/${campaign._id}`)}
      sx={{
        background: "linear-gradient(90deg, rgba(22, 7, 83, 0.61), rgba(81, 4, 61, 0.45))",
    ...
    <Box sx={{ display: "grid", gridTemplateColumns: gridTemplate, gap: 2, alignItems: "center", p: 2, minHeight: "80px" }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
                    <Avatar src={campaign.imageUrl} alt={campaign.title} variant="rounded" />
                    <Typography variant="subtitle1" fontWeight={700}>{campaign.title}</Typography>
                </Box>
                <Typography sx={{ color: getStatusColor(campaign.status), fontWeight: 700 }}>{campaign.status}</Typography>
                <Typography>{formatDate(campaign.startDate)}</Typography>
                <Typography>{formatDate(campaign.endDate)}</Typography> {/* <-- ADICIONADO AQUI */}
                <Typography fontWeight={500}>{displayPayment()}</Typography>
                <Typography fontWeight={500}>{campaign.applications || 0}</Typography>
                <Typography sx={{ color: getPrivacyColor(campaign.privacy), fontWeight: 700 }}>{campaign.privacy}</Typography>
                <Typography fontWeight={500}>{campaign.influencers || 0}</Typography>
            </Box>
    </Card>
  );
};
